...
@section('content')
<div class="col-md-8">
   {{-- membaca apakah ada pesan session dengan nama status --}}
   @if(session('status'))
   <div class="alert alert-success"> {{session('status')}}
   </div>
   @endif 

  {{-- menggunakan method selain GET dan POST 
dengan memberikan nilai method pada form sebagai POST,
 lalu tambahkan input bertipe hidden dengan nama _method bernilai nama method yang
akan digunakan Contohnya : <input name="_method" name="PUT">
   ...
   <form enctype="multipart/form-data" class="bg-white shadow-sm p-3"
      ...
      <br>
      <input
      {{-- mengecek apakah $user->roles didalamnya terdapat string "ADMIN" --}}
      {{-- menggunakan json_decode() untuk mengubah dari string JSON Array kembali ke PHP Array. --}}
      type="checkbox"
     ...
     <br>
      ...
      {{-- pengecekkan, jika user memiliki avatar kita tampilkan image avatarnya, jika tidak ada avatar kita tampilkan teks "No avatar" --}}
      @if($user->avatar)
 ...
 {{-- keterangan untuk mengabaikan field input ini jika tidak inginmengubah avatar --}}
      <hr class="my-4">
